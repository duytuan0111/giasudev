...
<h3 class="header">Quản lý url phụ huynh theo môn học và tỉnh thành</h3>
<form name="frmxoaall" id="frmxoaall" method="post" action="">
	<!-- <?php echo site_url('admin/del_vieclam'); ?> -->
	<!-- <p class="sidebar"> <input type="submit" name="submit" value="Xóa" /></p> -->
	<!--href="<?php echo site_url('admin/frmvieclam'); ?>"-->
	<table width="100%" style="display:block;">
		<tr class="title">
			...
			<td width="60%">Tên Page</td>
			<td width="50%">Url page</td>
			<td width="5%"></td>
			
		</tr>
		<?php foreach ($query as $n ) { ?>
		<?php foreach($query1 as $k) { ?>
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			<td align="center">
				<div id="request-form">
					<input type="checkbox" name="checkbox[]" class="checkbox" value="1" />
				</div>
			</td>
			<!-- <td align="center"><?php echo $n->ID; ?></td>					 -->
			<td><a >Tổng hợp tin tuyển gia sư <?php echo ($n->SubjectName); ?> tại <?php echo $k->cit_name ?></a></td>	<!--href="<?php echo site_url('admin/edit_vieclam/'.$item->UserID); ?>"-->		
			<td><a href="<?php echo base_url() ?>viec-lam-gia-su-mon-<?php echo vn_str_filter($n->SubjectName) ?>/tai-<?php echo vn_str_filter($k->cit_name) ?>-m<?php echo $n->ID; ?>c0p<?php echo $k->cit_id; ?>.html" target="blank"><?php echo base_url() ?>viec-lam-gia-su-mon-<?php echo vn_str_filter($n->SubjectName) ?>/tai-<?php echo vn_str_filter($k->cit_name) ?>-m<?php echo $n->ID; ?>c0p<?php echo $k->cit_id; ?>.html</td>			
			<td align="center">						
				<a href="<?php echo site_url('admin/edit_viectheomontinhthanh/'.$n->ID); ?>/<?php echo $k->cit_id; ?>" class="btn btn-sm btn-danger">Sửa</a>
			</td> 
