...
<form name="frmsearch" method="post" action="<?php echo site_url('admin/baiviettimgiasu'); ?>">
...
<form name="frmxoaall" id="frmxoaall" method="post" action="<?php echo site_url('admin/del_baiviettimgiasu'); ?>">
<p class="sidebar"><a href="<?php echo site_url('admin/frmbaiviettimgiasu'); ?>">Thêm mới</a> <input type="submit" name="submit" value="Xóa" /></p>
<table width="100%" style="display:block;">
    ...
    ?>
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			...
			<td><a href="<?php echo site_url('admin/edit_baiviettimgiasu/'.$item->id.'-'.url_title($item->alias)); ?>"><?php echo $item->title; ?></a></td>			
			<td align="center"><?php 
			...
			<td align="center">						
			  ...
			  ?>
			  <a class="status" onclick="check_status(<?php echo $item->id; ?>,'baiviettimgiasu','baiviettimgiasu')"><img src="images/toolbar/publish_y.png"></a>
			  <?php
			  }else{
			  ?>
			  <a class="status" onclick="check_status(<?php echo $item->id; ?>,'baiviettimgiasu','baiviettimgiasu')"><img src="images/toolbar/tick.png"></a>
			  <?php }
			  }
			  else
			  {
			  ?>
			  <a class="status" onclick="check_status(<?php echo $item->id; ?>,'baiviettimgiasu','baiviettimgiasu')"><img src="images/toolbar/publish_x.png"></a>
			  <?php
			  }
			  ?>
			</td>    			
			<td><?php 
					$cats=$CI->admin_model->gettbl('baiviettimgiasu',$item->id);
					if($cats->num_rows()> 0){
					$cat= $cats->row();
						echo $cat->link;
					}
