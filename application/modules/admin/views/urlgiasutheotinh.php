...
<h3 class="header">Quản lý url gia sư theo tỉnh thành</h3>
<form name="frmxoaall" id="frmxoaall" method="post" action=""> 
	...
	<table width="100%" style="display:block;">
		...
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			...
			<td><a >Hồ sơ gia sư dạy kèm tại <?php echo ($n->cit_name); ?></a></td>	<!--href="<?php echo site_url('admin/edit_vieclam/'.$item->UserID); ?>"-->		
			<td><a href="<?php echo base_url() ?>tai-<?php echo vn_str_filter($n->cit_name) ?>-m0l0t<?php echo $n->cit_id?>.html" target="blank"><?php echo base_url() ?>tai-<?php echo vn_str_filter($n->cit_name) ?>-m0l0t<?php echo $n->cit_id?>.html</a></td>			
			<td align="center">						
				<a href="<?php echo site_url('admin/edit_urltheotinh/'.$n->cit_id); ?>" class="btn btn-sm btn-danger">Sửa</a>
			</td> 
