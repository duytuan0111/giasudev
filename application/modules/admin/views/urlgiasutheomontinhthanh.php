...
<h3 class="header">Quản lý url gia sư theo môn học và tỉnh thành</h3>
<form name="frmxoaall" id="frmxoaall" method="post" action="">
	...
	<table width="100%" style="display:block;">
		...
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			...
			<td><a >hồ sơ gia sư dạy kèm <?php echo ($n->SubjectName); ?> tại <?php echo $k->cit_name ?></a></td>	<!--href="<?php echo site_url('admin/edit_vieclam/'.$item->UserID); ?>"-->		
			<td><a href="<?php echo base_url() ?>mon-<?php echo vn_str_filter($n->SubjectName) ?>/tai-<?php echo vn_str_filter($k->cit_name) ?>-s<?php echo $n->ID; ?>r0c<?php echo $k->cit_id; ?>.html" target="blank"><?php echo base_url() ?>mon-<?php echo vn_str_filter($n->SubjectName) ?>/tai-<?php echo vn_str_filter($k->cit_name) ?>-s<?php echo $n->ID; ?>r0c<?php echo $k->cit_id; ?>.html</a></td>			
			<td align="center">						
				<a href="<?php echo site_url('admin/edit_urltheomontinhthanh/'.$n->ID); ?>/<?php echo $k->cit_id; ?>" class="btn btn-sm btn-danger">Sửa</a>
			</td> 
