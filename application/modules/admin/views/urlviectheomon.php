...
<h3 class="header">Quản lý url phụ huynh theo môn học</h3>
<form name="frmxoaall" id="frmxoaall" method="post" action="">
	...
	<table width="100%" style="display:block;">
		...
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			...
			<td><a >Tổng hợp tin tuyển gia sư <?php echo ($n->SubjectName); ?></a></td>	<!--href="<?php echo site_url('admin/edit_vieclam/'.$item->UserID); ?>"-->		
			<td><a href="<?php echo base_url() ?>viec-lam-gia-su-mon-<?php echo vn_str_filter($n->SubjectName) ?>-s<?php echo $n->ID?>c0p0.html" target="blank"><?php echo base_url() ?>viec-lam-gia-su-mon-<?php echo vn_str_filter($n->SubjectName) ?>-s<?php echo $n->ID?>c0p0.html</td>			
			<td align="center">						
				<a href="<?php echo site_url('admin/edit_viectheomon/'.$n->ID); ?>" class="btn btn-sm btn-danger">Sửa</a>
			</td> 
