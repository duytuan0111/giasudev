<h3 class="header">Sửa url phụ huynh</h3>
<div class="content-inner">
	<form name="frmslider" action="<?php echo site_url('admin/exit_urlphuhuynh'); ?>" method="post" enctype="multipart/form-data">
		<?php 
			if(isset($id))
			{
		...
		<tr>
			...
			<td><select name="option" id="option">
				<option value="0" <?php if (isset($id) && ($item->option == 0)) { echo 'selected';} ?> >Môn học</option>
				<option value="1" <?php if (isset($id) && ($item->option == 1)) { echo 'selected';} ?> >Địa điểm</option>
	...
	$(document).ready(function() {
		$('.submit').click(function(event) {
			...
			if (h1 == '') {
				...
				$.ajax({
					url: url+"admin/edit_url_phuhuynh",
					type: "POST",
					dataType: "JSON",
