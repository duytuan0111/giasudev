...
<h3 class="header">Quản lý url phụ huynh theo tỉnh thành</h3>
<form name="frmxoaall" id="frmxoaall" method="post" action=""> 
	...
	<table width="100%" style="display:block;">
		...
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			...
			<td align="center"><?php echo $n->cit_id; ?></td>					
			<td><a >Tổng hợp tin tuyển gia sư tại <?php echo ($n->cit_name); ?></a></td>	<!--href="<?php echo site_url('admin/edit_vieclam/'.$item->UserID); ?>"-->		
			<td><a href="<?php echo base_url() ?>viec-lam-gia-su-tai-<?php echo vn_str_filter($n->cit_name) ?>-s0c0p<?php echo $n->cit_id?>.html" target="blank"><?php echo base_url() ?>viec-lam-gia-su-tai-<?php echo vn_str_filter($n->cit_name) ?>-s0c0p<?php echo $n->cit_id?>.html</td>			
			<td align="center">						
				<a href="<?php echo site_url('admin/edit_viectheotinh/'.$n->cit_id); ?>" class="btn btn-sm btn-danger">Sửa</a>
			</td> 
